const cfg={supabaseUrl:'https://rtnigoszuttbmyvzgscu.supabase.co',supabaseKey:'sb_publishable_tnlE3wVwIT8RHkPXJb5-jQ_MxlwhnKR'};
...
async function aplicarSessao(session){
 ...
 const admin=(currentUser.email||'').toLowerCase()==='pedro.rosa@itu.sp.gov.br'||currentProfile?.funcao==='administrador';
 document.getElementById('adminNav').style.display=admin?'inline-block':'none';
 try{await sb.from('perfis').update({ultimo_acesso:new Date().toISOString()}).eq('id',currentUser.id);}catch(e){}
}
...
async function log(tipo,detalhes,extra={}){if(!sb||!currentUser)return;try{await sb.from('atividades').insert({usuario_id:currentUser.id,tipo,igreja:extra.igreja||null,cidade:extra.cidade||null,detalhes});}catch(e){console.warn('log',e.message);}}
async function carregarAdmin(){
 if(!sb||!currentUser)return;let users=[],logs=[],visitas=[];try{const r=await sb.from('perfis').select('*').limit(200);users=r.data||[];}catch(e){}try{const r=await sb.from('atividades').select('*').order('criado_em',{ascending:false}).limit(100);logs=r.data||[];}catch(e){}try{const r=await sb.from('visitas').select('*').order('visitada_em',{ascending:false}).limit(100);visitas=r.data||[];}catch(e){}
 document.getElementById('admUsuarios').textContent=users.length;document.getElementById('admAtividades').textContent=logs.length;document.getElementById('admVisitas').textContent=visitas.length;
 document.getElementById('adminUsers').innerHTML=users.length?'<table><tr><th>Usuário</th><th>Perfil</th><th>Equipe</th><th>Carro</th><th>Último acesso</th></tr>'+users.map(u=>`<tr><td>${esc(u.nome||u.email||u.id)}</td><td>${esc(u.funcao||'equipe')}</td><td>${esc(u.equipe||'')}</td><td>${esc(u.carro||'')}</td><td>${u.ultimo_acesso?new Date(u.ultimo_acesso).toLocaleString('pt-BR'):'—'}</td></tr>`).join('')+'</table>':'<p class="muted">Nenhum perfil encontrado.</p>';
 document.getElementById('adminLogs').innerHTML=logs.length?'<table><tr><th>Data</th><th>Tipo</th><th>Descrição</th></tr>'+logs.map(l=>`<tr><td>${new Date(l.criado_em).toLocaleString('pt-BR')}</td><td>${esc(l.tipo)}</td><td>${esc(l.detalhes||'')}</td></tr>`).join('')+'</table>':'<p class="muted">Nenhuma atividade registrada.</p>';
}
...
async function marcarVisita(i){const all=window.IGREJAS||[];const x=all[i];if(!x)return;x.visitada=!x.visitada;drawChurches();if(x.visitada){try{await sb.from('visitas').insert({usuario_id:currentUser.id,igreja:x.nome||x.igreja,cidade:x.cidade,endereco:x.endereco||x.bairro||null,equipe:currentProfile?.equipe||null,carro:currentProfile?.carro||null,status:'visitada'});}catch(e){}await log('visita','Igreja marcada como visitada',{igreja:x.nome||x.igreja,cidade:x.cidade});}}
